...
export default function OpeningRevealOverlay({
  ...
  // Prefer the invitation's own uploaded cover over the template-level opening
  // config image (which defaults to a shared placeholder), so real invitations
  // don't fall back to the default photo in the opening.
  const revealCoverImage = coverImage || config.coverImage || coverConfig.coverImage || "/assets/CoverPasangan.png";
  ...
  const usePhotoBackdrop =
    Boolean(config.coverImageEnabled && revealCoverImage) && !isSplitAnimation && animation !== "paper";
  ...
  const coverImageClass = compactMode
    ? "mx-auto mb-5 aspect-[3/4] w-36 rounded-t-full rounded-b-[14px] object-cover shadow-xl shadow-[var(--color-primary)]/12"
    : "mx-auto mb-7 aspect-[3/4] w-44 rounded-t-full rounded-b-[16px] object-cover shadow-2xl shadow-[var(--color-primary)]/12";
  ...
  const guestBoxClass = compactMode
    ? "mx-auto mt-5 max-w-[250px] rounded-[8px] border border-[var(--color-accent-pale)] bg-[var(--color-surface)]/88 px-5 py-4 shadow-xl shadow-[var(--color-primary)]/10 backdrop-blur"
    : "mx-auto mt-8 max-w-md rounded-[8px] border border-[var(--color-accent-pale)] bg-[var(--color-surface)]/88 px-7 py-5 shadow-xl shadow-[var(--color-primary)]/10 backdrop-blur";
  const guestEyebrowClass = compactMode
    ? "text-[10px] font-black uppercase tracking-[0.14em] text-[var(--color-accent)]"
    : "text-sm font-black uppercase tracking-[0.16em] text-[var(--color-accent)]";
  const guestNameClass = compactMode
    ? "mt-1 text-lg font-black text-[var(--color-primary)]"
    : "mt-2 text-2xl font-black text-[var(--color-primary)]";
  ...
  return (
    <motion.div exit={revealExitMotion(animation)} transition={{ duration: 0.72, ease: "easeInOut" }} className={`fixed inset-0 z-[120] flex h-[100dvh] min-h-[100svh] items-center justify-center overflow-hidden ${overlaySpacingClass} text-center ${frameClass}`} style={{ backgroundColor }}>
      {isSplitAnimation ? (
        <>
      ...
      {usePhotoBackdrop ? (
        <>
          <img src={revealCoverImage} alt="" className="absolute inset-0 z-0 h-full w-full object-cover" />
      ...
      <OpeningSequence config={config} isOpening={isOpening} className={`relative z-10 mx-auto w-full ${contentWidthClass} ${contentLayoutClass} ${contentClass}`}>
        {config.coverImageEnabled && !usePhotoBackdrop ? <img src={revealCoverImage} alt={`${couple.groomNickname} dan ${couple.brideNickname}`} className={coverImageClass} /> : null}
